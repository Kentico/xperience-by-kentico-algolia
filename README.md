...
<a href="https://raw.githubusercontent.com/Kentico/xperience-by-kentico-algolia/main/images/main-menu.png">
  <img src="https://raw.githubusercontent.com/Kentico/xperience-by-kentico-algolia/main/images/main-menu.png" width="600" alt="Administration search index list">
</a>
...
1. Define a custom `AlgoliaSearchModel` implementation to represent the content you want to index.
1. Override the search model `OnIndexingProperty` method to customize the model's content before indexing.
...
1. Add this library to the application services, registering your custom `AlgoliaSearchModel`, using `builder.Services.AddAlgolia()`:
   ...
   builder.Services.AddAlgolia(builder.Configuration, new[]
   {
       new AlgoliaIndex(typeof(MySearchModel), SiteSearchModel.IndexName)
   });
   ```

1. Rebuild the index in Xperience's Administration within the Search application added by this library.
1. Use the `IAlgoliaIndexService` (via DI) to retrieve the index populated by your custom `AlgoliaSearchModel`.
1. Execute a search with a customized Algolia `Query` using the `ISearchIndex`.
